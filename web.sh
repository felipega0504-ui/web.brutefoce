...
# Input interactivo o por argumento
if [ $# -lt 1 ]; then
    echo -n "Web URL: "
    read URL
    if [ -z "$URL" ]; then
...
echo ""
echo "=== Monitor de usuarios conectados o registrados (usuarios.txt) ==="
echo ""

USUARIOS_ARCHIVO="usuarios.txt"
if [ ! -f "$USUARIOS_ARCHIVO" ]; then
    touch "$USUARIOS_ARCHIVO"
fi

while true; do
    clear
    echo "Usuarios registrados o conectados (actualizado: $(date '+%Y-%m-%d %H:%M:%S')):"
    if [ -s "$USUARIOS_ARCHIVO" ]; then
        cat "$USUARIOS_ARCHIVO"
    else
        echo "No hay usuarios registrados o conectados listados."
    fi
    echo ""
    echo "[Se actualizará automáticamente cada 1 minuto. Pulsa Ctrl+C para salir.]"
    sleep 60
done
